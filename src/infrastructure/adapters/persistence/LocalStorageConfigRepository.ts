...
export class LocalStorageConfigRepository implements ConfigRepositoryPort {
  ...
  getTTSConfig(): Promise<Result<TTSConfigProps | null, Error>> {
    try {
      const data = localStorage.getItem(STORAGE_KEYS.TTS_CONFIG);
      if (!data) return Promise.resolve(ok(null));

      const config = JSON.parse(data) as TTSConfigProps;
      return Promise.resolve(ok(config));
    } catch (error) {
      return Promise.resolve(err(error instanceof Error ? error : new Error('Failed to load TTS config')));
    }
  }

  saveTTSConfig(config: TTSConfigProps): Promise<Result<void, Error>> {
    try {
      localStorage.setItem(STORAGE_KEYS.TTS_CONFIG, JSON.stringify(config));
      return Promise.resolve(ok(undefined));
    } catch (error) {
      return Promise.resolve(err(error instanceof Error ? error : new Error('Failed to save TTS config')));
    }
  }
  ...
  resetToDefaults(): Promise<Result<void, Error>> {
    try {
      ...
      localStorage.removeItem(STORAGE_KEYS.TTS_CONFIG);
      return Promise.resolve(ok(undefined));
    } catch (error) {
